...
def openSerialInterface(device):
    ser = serial.Serial(port=device.device_node,baudrate=57600,parity=serial.PARITY_NONE,stopbits=serial.STOPBITS_ONE,timeout=1)
    ...
    lcd.displayMessage('ID: ' + currentDevice.shortId, 2)
    return ser
...
def blinkLED(device, ser):
    timeout = 0.1
    message = 'blinking LED...'
    print(message)
    lcd.displayMessage(message, 1)
...
def packetTest(device, ser):
    ...
    for x in range(numpacks):
        ...
        if re.search(regex, response):
            ...
            lcd.displayMessage('PKTS:{0}/{1}'.format(oks, numpacks), 1)
        time.sleep(timeout)
    if oks == numpacks:
        print('PASSED ({0}/{1})'.format(oks,numpacks))
        lcd.displayMessage('PKTS:{0}/{1}      OK'.format(oks, numpacks), 1)
    else:
        print('FAILED ({0}/{1}) consider using -v for more info'.format(oks,numpacks))
        lcd.displayMessage('PKTS:{0}/{1}    FAIL'.format(oks, numpacks), 1)

def device_event(action, device):
    if action == 'add':
        lcd.displayMessage('JEELINK ADDED', 1)
...
def main():
    ...
    setConfiguration()      #query configuration and set remote properties for testing against
